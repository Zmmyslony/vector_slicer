...
DesiredPattern::DesiredPattern(std::string& shapeFilename, std::string& xVectorFieldFilename, std::string& yVectorFieldFilename):
...
std::valarray<double> DesiredPattern::preferredDirection(std::valarray<double> position, int distance) {
    ...
    if (xField == 0 && yField == 0) {
        return std::valarray<double> {xFieldPreferred[xPosition - 1][yPosition], yFieldPreferred[xPosition - 1][yPosition]};
    }
