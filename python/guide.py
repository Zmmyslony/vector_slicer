...
from lib.director import basic_directors
from lib.shape import basic_shapes
...
def complete_usage_example(slicer):
    ...
    :return:
    """
    # Define domains that are used for pattern generation. See basic_shapes.py for predefined shapes.
    rectangle = basic_shapes.rectangle(0, 0, 20, 10)

    # Define director patterns used for pattern generation. See basic_directors.py for predefined directors.
    uniaxial_x_director = basic_directors.uniaxial_alignment(0)
...
def shape_addition(line_width):
    ...
    x_rectangle = basic_shapes.rectangle(-10, -5, 10, 5)
    y_rectangle = basic_shapes.rectangle(-5, -10, 5, 10)
...
def shape_symmetrisation(line_width):
    ...
    x_rectangle = basic_shapes.rectangle(0, -5, 20, 5)
...
def pattern_addition(line_width):
    ...
    uniaxial_x_director = basic_directors.uniaxial_alignment(0)
    azimuthal_director = basic_directors.azimuthal()
...
def pattern_symmetrisation(line_width):
    ...
    x_rectangle = basic_shapes.rectangle(0, -5, 20, 5)
    disk = basic_shapes.disk(2.5, 20, 5)
...
def example_extensive():
    ...
    # Define domains that are used for pattern generation. See basic_shapes.py for predefined shapes.
    annulus = basic_shapes.annulus(5, 10)
    disk = basic_shapes.disk(10)
    rectangle = basic_shapes.rectangle(0, 0, 20, 10)
    square = basic_shapes.rectangle(-10, -10, 10, 10)

    # Domains can be added or subtracted from another to create new shapes.
    cross = basic_shapes.rectangle(-30, -10, 30, 10) + basic_shapes.rectangle(-10, -30, 10, 30)
    intersection_square_disk = square - disk

    # Define director patterns used for pattern generation. See basic_directors.py for predefined directors.
    uniaxial_longitudinal_director = basic_directors.uniaxial_alignment(0)
    uniaxial_diagonal_director = basic_directors.uniaxial_alignment(np.pi / 4)
    uniaxial_transverse_director = basic_directors.uniaxial_alignment(np.pi / 2)

    radial_alignment = basic_directors.radial()
    azimuthal_director = basic_directors.azimuthal()
    three_charge_field_alignment = basic_directors.charge_field([[-5, -5], [-5, 5], [5, 3]], [1, -1, 1])
